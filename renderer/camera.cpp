...
Point Camera::ConvertToScreen(const Point& worldPt, const WorldLimits& limits) const
{
  ...
  const auto screen_x = m_window_width * (horizontal_len / world_width);
  const auto screen_y = m_window_height * (world_height - vertical_len) / world_height;
  return { screen_x, screen_y };
}

Point Camera::ConvertToWorld(const Point& screenPt, const WorldLimits& limits) const
{
  f64 norm_x = screenPt.x / m_window_width;
  f64 norm_y = screenPt.y / m_window_height;
